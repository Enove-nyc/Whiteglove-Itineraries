...
describe("the policy names what the site actually loads", () => {
  ...
  it("GRANTS unsafe-eval and blob: on script-src — the map needs them", () => {
    // Google Maps' WebGL renderer compiles code with eval() and loads a worker
    // over a blob: URL. `new google.maps.Map()` does not throw when those are
    // blocked, so the map never falls back to OpenStreetMap — it just paints
    // blank. Both are documented by Google as required.
