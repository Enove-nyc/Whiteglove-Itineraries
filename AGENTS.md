...
- **Forwarding a booking in by email is wired, and the DNS behind it is not obvious.** White Glove Itineraries receives at **`trips.whitegloveitineraries.com`** — a subdomain, and it had to be. The root `whitegloveitineraries.com` and `whiteglovekoshertravel.com` both carry Cloudflare Email Routing MX records for the owner's real mail, and `mail.whitegloveitineraries.com` is delegated to Brevo's own nameservers. A receiving MX on any of those three takes over mail that is already working. Resend's Cloudflare integration wrote the records for the new subdomain itself, so there was nothing to add by hand.
  ...
  THE PROVIDER SIGNS WITH SVIX, NOT A PLAIN HMAC. `/api/inbound/confirmation` verifies `id.timestamp.body`, keyed on the base64-DECODED part of the secret after `whsec_`, digest in base64, header a SPACE-separated list of `v1,<sig>`. The first version hashed the body alone in hex and would have rejected every real delivery — silently, because the route fails closed, so mail would have arrived at the mailbox and simply never appeared in anybody's queue. Do not "simplify" that verifier.

  WHERE THE SETTINGS LIVE: `INBOUND_EMAIL_SECRET` and `INBOUND_EMAIL_DOMAIN` are Railway variables on the itineraries service. **Secrets are never written into this file, into any file in either repository, or into a commit message.** If a key is ever needed, ask for it and set it in Railway; the value belongs there and nowhere else.

- **Each front door sends from its own domain, and a mismatch is now caught.** Sign up on Itineraries and the code comes from an itineraries address; sign up on Kosher Travel and it comes from a kosher one. `postResend` resolves `senderForBrand(currentBrand())` for every send, and `senderMismatch` (lib/email.ts) refuses to let the wrong domain pass unnoticed — it logs on every send and shows in red on the admin email panel with both brands' real sender addresses. A subdomain such as `send.example.com` counts as correct; a lookalike domain does not. It warns rather than blocks, because mail from the wrong name still beats mail that never arrives. The failure it exists for is silent in every other way: Resend answers 200 and the delivery log records a success.
